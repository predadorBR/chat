$(function() {
  ...
  function notify(change, numUsers) {
    ...
    if (numUsers === 1) {
      if (roomId != 'lobby') {
        msg += 'Esta é uma sala de chat privada. Por favor, convide outras pessoas enviando-lhes este link.';
      } else {
        msg += 'Agora há 1 participante.';
      }
    } else {
      msg += 'Há agora ' + numUsers + ' participantes.';
    }
    //format
  ...
  function removeRoomFromList(roomName) {
    ...
    .fail(function(){
      alert('Erro ao excluir esta sala');
    });
  }
  ...
  socket.on('username failed', function(){
    alert('Este nome de usuário já foi usado. Por favor escolha um diferente.');
  })
  ...
  socket.on('roomName failed', function(){
    alert('Você já tem um quarto com este nome');
  });
  ...
  socket.on('cannot remove room', function(){
    alert('Você não pode excluir a sala em que está atualmente.');
  });
  ...
  socket.on('user joined', function(data){
    var change = data.username + ' Entrou.';
    notify(change, data.numUsers);
  });

  socket.on('user left', function(data){
    var change = data.username + ' Saiu.';
    notify(change, data.numUsers);
